...
export default function RenderHome(props) {
    ...
    useEffect(() => {
        async function getInVotingProposals() {
            ...
            let inVotingList = await getProposals(
        ...
        async function getSubmittedProposals() {
            ...
            let submittedList = await getProposals(
                GLOBAL_VARS.BY_STAT_CAT_QUERY_TYPE,
                GLOBAL_VARS.SUBMITTED_KEY,
                getStatBounds
            );
        ...
        async function getCompletedProposals() {
            ...
            let completedList = await getProposals(
                GLOBAL_VARS.BY_STAT_CAT_QUERY_TYPE,
                GLOBAL_VARS.COMPLETED_KEY,
                getStatBounds
            );
        ...
        async function getInProgressProposals() {
            ...
            let inProgressList = await getProposals(
                GLOBAL_VARS.BY_STAT_CAT_QUERY_TYPE,
            ...
            do {
                try {
                    ...
                    if (resp.rows) {
                        ...
                        success = true;
                        configData.available_funds = requestedAmountToFloat(
                            configData.available_funds
                        );
                        configData.display_available_funds =
                            numberWithCommas(configData.available_funds).toString() + ' WAX';
                    }
                    ...
                    if (savingsResp.rows) {
                        console.debug(savingsResp.rows);
                        additional_funds = savingsResp.rows[0].balance;
                        configData.additional_funds = requestedAmountToFloat(additional_funds);
                        configData.display_additional_funds =
                            numberWithCommas(configData.additional_funds).toString() + ' WAX';
                    }
    ...
    return (
        <div className="home">
            <div className="home__cover">
                ...
                <Link
                    to={GLOBAL_VARS.PROPOSALS_IN_VOTING_LINK}
                ...
                >
                    <Nav className="home__tabs">
                        ...
                        >
                            <span className="tag tag--attention">
                                {
                                    GLOBAL_VARS.READABLE_PROPOSAL_STATUS[
                                        GLOBAL_VARS.PROPOSAL_INPROGRESS_KEY
                                    ]
                                }
                            </span>
                    ...
                    <Tab.Content className="home__content">
                        ...
                        <Tab.Pane
                            className="home__tabPane"
                            eventKey="drafting"
                        >
                            <img
                                src={drafting}
                                alt="Flow for a drafting proposal."
                            />
                        </Tab.Pane>
                        <Tab.Pane
                            className="home__tabPane"
                        ...
                        >
                            <img
                                src={review}
                                alt="Flow for a proposal that is in review."
                            />
                        </Tab.Pane>
                        <Tab.Pane
                            className="home__tabPane"
                        ...
                        >
                            <img
                                src={approved}
                                alt="Flow for an approved proposal."
                            />
                        </Tab.Pane>
                        <Tab.Pane
                            className="home__tabPane home__tabPane--large"
                            eventKey="voting"
                        >
                            <h1>{configData.voting_duration}</h1>
                            <VotingProposal
                                votingDuration={calculateTime(configData.vote_duration)}
                            />
                        </Tab.Pane>
                        <Tab.Pane
                            className="home__tabPane"
                        ...
                        >
                            <img
                                src={progress}
                                alt="Flow for a proposal that is in progress."
                            />
                        </Tab.Pane>
                        <Tab.Pane
                            className="home__tabPane"
